...
//    }
    ...
    public function changePasswordAction($type) {
        $contentFirst = $this->getRequest()->getContent();
        $contentRtrim = rtrim($contentFirst, ',');
        $content = explode(',', $contentRtrim);

        if ($content[1] !== $content[2]) {
            return new Response('<font style="color:red; font-size:12px">New Password and confirm password <br> must be the same.</font>');
        }
        $em = $this->getDoctrine()->getEntityManager();
        $entity = $this->getCurUser();
        $pw = $entity->getPassword();
        ...
        switch ($type) {
            case 'platform':
                if (sha1($content[0]) == $pw || ($content[0] == '' && $pw == '')){
                ...
                break;
            case 'aol':
                if ($content[0] == $aolpw || ($content[0] == '' && $aolpw == '')) {
                ...
                break;
            case 'ace':
                if ($content[0] == $acepw || ($content[0] == '' && $acepw == '')) {
                ...
                break;
            case 'crm':
                if ($content[0] == $crmpw || ($content[0] == '' && $crmpw == '')) {
    ...
    public function editProfileByAjaxAction() {
        $contentJson = $this->getRequest()->getContent();
        $content = json_decode($contentJson, true);
        $userEntity = $this->getCurUser();

        $em = $this->getDoctrine()->getManager();

        $userEntity->setUsername(trim($content['platformname-td']));
        $userEntity->setAolUsername(trim($content['aolname-td']));
        $userEntity->setAceUsername(trim($content['acename-td']));
        $userEntity->setCrmUsername(trim($content['crmname-td']));
        $userEntity->setFullname(trim($content['fullname-td']));
        $userEntity->setEmail(trim($content['email-td']));
        $userEntity->setTitle(trim($content['title-td']));
        $userEntity->setSkype(trim($content['skype-td']));
        $userEntity->setMobile(trim($content['mobile-td']));
        $userEntity->setOfficePhone(trim($content['officephone-td']));
        $userEntity->setCountry(trim($content['country-td']));
        $userEntity->setOfficeAddress(trim($content['officeaddress-td']));
    ...
    public function savePicAction() {
        ...
        $picPath = $entity->getWebPath();
        $em->persist($entity);
        $em->flush();
        
        return new Response($picPath);
    }
}